...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        #profiler.profile_name_scope(options=(option_builder.ProfileOptionBuilder.trainable_variable_parameter()))

        # the timing of the model op
        #options = option_builder.ProfileOptionBuilder.time_and_memory()
        #profiler.profile_operations(options=options)

        # timeline
        options = option_builder.ProfileOptionBuilder.time_and_memory()
        profiler.profile_graph(options=options)
        ...
        for i in range(3):
            ...
            with open('timeline_02_op_step_%d.json' % i, 'w') as f:
